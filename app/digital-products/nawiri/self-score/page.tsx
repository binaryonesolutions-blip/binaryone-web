...
export default function LoyaltySelfScorePage() {
  return (
    <div
      data-skin="nawiri"
      className="mx-auto min-h-screen w-[1440px] bg-[#0A1628] font-inter text-white [background-image:linear-gradient(rgba(45,212,191,0.05)_1px,transparent_1px),linear-gradient(90deg,rgba(45,212,191,0.05)_1px,transparent_1px)] [background-size:48px_48px]"
    >
      {/* Mini general nav */}
      <div className="flex h-[40px] items-center justify-between bg-[#071120] px-[64px]">
        <Link href={routes.home} className="font-inter text-[12px] font-semibold text-[#94A3B8] hover:text-[#9EFF5A]">← binaryone.co.ke</Link>
        <nav className="flex items-center gap-[24px] font-inter text-[12px] font-medium">
      ...
      {/* Header */}
      <header className="sticky top-0 z-50 flex h-[76px] items-center justify-between border-b border-[rgba(45,212,191,0.25)] bg-[rgba(10,22,40,0.92)] px-[64px] [backdrop-filter:blur(12px)]">
        <div className="flex items-center gap-[16px]">
          <Link href={routes.nawiri} className="flex items-baseline gap-[8px]">
            <span className="font-sora text-[20px] font-extrabold tracking-[0.02em] text-white">NAWIRI</span>
            <span className="font-inter text-[10px] font-medium tracking-[0.16em] text-[#94A3B8]">DIGITAL LOYALTY</span>
          </Link>
          <span className="h-[22px] w-[1px] bg-[rgba(45,212,191,0.4)]" />
          <span className="font-inter text-[12px] font-medium text-[#94A3B8]">Loyalty Programme Maturity Self-Score</span>
        </div>
        <nav className="flex items-center gap-[32px] font-inter text-[14.5px] font-medium">
          <Link href={routes.nawiri} className="text-[#E2E8F0]">← Back to NAWIRI</Link>
      ...
      {/* Intro */}
      <section className="max-w-[900px] px-[64px] pb-[48px] pt-[80px]">
        ...
        <h1 className="mb-[20px] font-sora text-[46px] font-extrabold leading-[1.1] text-white [text-wrap:pretty]">How mature is your loyalty programme, <span className="text-[#9EFF5A]">honestly?</span></h1>
        <p className="font-inter text-[17px] leading-[1.65] text-[#94A3B8] [text-wrap:pretty]">Score each statement from 1 (not true of us) to 4 (fully true of us). Your total places your programme in one of four maturity eras — and tells you what to fix first before investing further.</p>
      </section>

      <LoyaltySelfScore />

      {/* Footer strip */}
      <footer className="flex flex-col items-center gap-[18px] border-t border-[rgba(45,212,191,0.25)] px-[64px] pb-[32px] pt-[28px]">
        <div className="flex flex-wrap items-center justify-center gap-[26px]">
          <Link href={routes.nawiri} className="inline-flex items-center gap-[9px] font-inter text-[14.5px] font-semibold text-[#9EFF5A] hover:text-[#c6ff9d]"><span className="text-[16px]">←</span>Back to NAWIRI</Link>
