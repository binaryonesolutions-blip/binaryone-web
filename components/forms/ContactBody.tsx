...
function advisoryDates(): string[] {
  ...
  while (out.length < 5) {
    ...
    out.push(d.toLocaleDateString("en-GB", { weekday: "short", month: "short", day: "numeric" }).replace(/^(\w+)\s/, "$1, "));
  }
  return out;
}
    ...
    <div className="grid grid-cols-1 lg:grid-cols-[1.35fr_0.65fr] items-start gap-[48px] px-5 sm:px-8 lg:px-[64px] pb-[104px] pt-[56px]">
      ...
      <div>
        <div className="mb-[32px] flex rounded-[12px] border border-[#E5E7EB] bg-[#f0eded] p-[4px]">
          <button onClick={() => { setTab("assessment"); setSubmitted(false); }} className={tabBtn(tab === "assessment")}>Book a Free IT Assessment</button>
          <button onClick={() => { setTab("enquiry"); setSubmitted(false); }} className={tabBtn(tab === "enquiry")}>General Enquiry</button>
        </div>
          ...
          <div className="rounded-[16px] border border-[#E5E7EB] bg-white p-[40px] shadow-[0_1px_2px_rgba(0,0,0,0.04),0_8px_24px_rgba(0,0,0,0.05)]">
            ...
            <div className="flex flex-col gap-[16px]">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-[16px]">
                ...
                <input placeholder="Preferred meeting date *" type="date" className={`${FIELD} py-[12px]`} />
              </div>
              <textarea placeholder="Anything we should know? (optional)" rows={2} className={`${FIELD} resize-y`} />
          ...
          <div className="rounded-[16px] border border-[#E5E7EB] bg-white p-[40px] shadow-[0_1px_2px_rgba(0,0,0,0.04),0_8px_24px_rgba(0,0,0,0.05)]">
            ...
            <div className="flex flex-col gap-[16px]">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-[16px]">
                ...
                <input placeholder="Role / Title *" className={FIELD} />
                <input placeholder="Work email *" type="email" className={FIELD} />
                <input placeholder="Phone (optional)" type="tel" className={FIELD} />
                <select className={SELECT} defaultValue="">
              ...
              <textarea placeholder="Message *" rows={5} className={`${FIELD} resize-y`} />
              <label className="flex cursor-pointer items-start gap-[10px] font-inter text-[13px] leading-[1.5] text-[#3e4947]">
                <input type="checkbox" className="mt-[2px] [accent-color:#006e1b]" />
      ...
      <div className="flex flex-col gap-[20px]">
        <div className="flex items-stretch gap-[22px] rounded-[16px] border border-[rgba(56,224,196,0.75)] bg-[#d9f5ef] p-[32px] shadow-[0_0_0_1px_rgba(56,224,196,0.18),0_0_28px_rgba(56,224,196,0.30),0_18px_40px_-30px_rgba(6,35,30,0.35)]">
          ...
          <div className="flex flex-1 flex-col gap-[16px]">
            ...
            <button onClick={() => { setAdvisoryOpen(true); setAdvDone(false); setPartnersOpen(false); }} className="inline-flex cursor-pointer items-center gap-[9px] self-start whitespace-nowrap rounded-[9px] border border-[#00332f] bg-[#00332f] px-[14px] py-[9px] font-jet text-[10.5px] font-bold tracking-[0.1em] text-[#eafaf6] transition-[background,border-color] hover:border-[#0f766e] hover:bg-[#0a4a42] hover:text-white">
            ...
            {advDone ? (
              ...
              <div className="flex flex-col gap-[12px]">
                <div className="relative flex flex-col gap-[6px]">
                  ...
                  <button onClick={() => setPartnersOpen((v) => !v)} className="flex w-full cursor-pointer items-center justify-between gap-[12px] rounded-[10px] border border-white/[0.16] bg-white/[0.05] px-[14px] py-[10px] text-left font-inter text-[14px] text-white hover:border-[rgba(56,224,196,0.55)]">
                ...
                <div className="flex flex-col gap-[6px]">
                  ...
                  <div className="flex flex-nowrap gap-[8px]">
                    {dates.map((label, i) => (
                      <button key={label} onClick={() => setAdvDate(i)} className="flex-1 cursor-pointer whitespace-nowrap rounded-[9px] border px-[10px] py-[9px] font-inter text-[13px] font-semibold hover:border-[#38e0c4]" style={chip(i === advDate)}>{label}</button>
                    ))}
                  </div>
                </div>
                ...
                <div className="flex flex-col gap-[6px]">
                  ...
                  <div className="flex flex-nowrap gap-[8px]">
                    {ADV_SLOTS.map((label, i) => (
                      <button key={label} onClick={() => setAdvSlot(i)} className="flex-1 cursor-pointer whitespace-nowrap rounded-[9px] border px-[10px] py-[8px] font-inter text-[13px] font-semibold tracking-[0.01em] hover:border-[#38e0c4]" style={chip(i === advSlot)}>{label}</button>
                    ))}
                  </div>
                </div>
                ...
                <div className="grid grid-cols-2 lg:grid-cols-4 gap-[10px]">
                  ...
                  <label className="flex flex-col gap-[6px]"><span className={ADV_LABEL}>MOBILE NUMBER</span><input type="tel" placeholder="+254 700 000 000" className={ADV_FIELD} /></label>
                </div>

                <label className="flex flex-col gap-[6px]">
                  <span className={ADV_LABEL}>BRIEFING SCOPE</span>
                  <textarea rows={1} placeholder="vCIO, ERP advisory, NAWIRI, Agentic AI or Custom Software — and the decision you need to make." className={`${ADV_FIELD} resize-y leading-[1.55]`} />
                </label>

                <button onClick={() => setAdvDone(true)} className="w-full cursor-pointer rounded-[12px] border-none bg-[#38e0c4] px-[24px] py-[13px] font-sora text-[14.5px] font-bold tracking-[0.03em] text-[#06231e] hover:bg-[#5eead4]">Secure Advisory Reservation</button>
