...
export default function ContactBody() {
  ...
  // Boardroom Advisory validation (31AUG design): email + phone + consent required.
  const [advConsent, setAdvConsent] = useState(false);
  const [advErrEmail, setAdvErrEmail] = useState(false);
  const [advErrPhone, setAdvErrPhone] = useState(false);
  const [advErrConsent, setAdvErrConsent] = useState(false);
  ...
  function resetValidation() {
    ...
    setErrEmail(false); setErrPhone(false); setErrConsent(false);
  }
  ...
  async function handleForm(
    ...
    const okC = consent;
    if (!okE || !okP || !okC) { setErrEmail(!okE); setErrPhone(!okP); setErrConsent(!okC); return; }
  ...
  async function handleAdvisory(e: React.FormEvent<HTMLFormElement>) {
    e.preventDefault();
    const fd = new FormData(e.currentTarget);
    const okE = String(fd.get("email") || "").indexOf("@") > 0;
    const okP = String(fd.get("phone") || "").replace(/[^0-9]/g, "").length >= 9;
    if (!okE || !okP || !advConsent) {
      setAdvErrEmail(!okE); setAdvErrPhone(!okP); setAdvErrConsent(!advConsent);
      return;
    }
    setAdvErrEmail(false); setAdvErrPhone(false); setAdvErrConsent(false);
    setError("");
    setSending(true);
    try {
  ...
  const contactCard = (
    <div className="flex items-stretch gap-[16px] sm:gap-[22px] rounded-[16px] border border-[rgba(56,224,196,0.75)] bg-[#d9f5ef] p-[22px] sm:p-[32px] shadow-[0_0_0_1px_rgba(56,224,196,0.18),0_0_28px_rgba(56,224,196,0.30),0_18px_40px_-30px_rgba(6,35,30,0.35)]">
      ...
      <div className="flex flex-1 flex-col gap-[16px]">
        ...
        <div className="font-inter text-[15px] leading-[1.75] text-[#2c4b46]">
          <strong className="font-bold text-[#06332e]">Binary One Solutions Ltd</strong>
          <span className="mt-[2px] flex items-start gap-[7px]">
            <svg viewBox="0 0 24 24" className="mt-[6px] h-[13px] w-[13px] flex-shrink-0 opacity-75" fill="none" stroke="currentColor" strokeWidth="1.7" strokeLinejoin="round"><path d="M12 22s7-6.1 7-12a7 7 0 1 0-14 0c0 5.9 7 12 7 12Z" /><circle cx="12" cy="10" r="2.4" /></svg>
            <span>St Charles Lwanga House, 1st Flr,<br />Ngong Road, Nairobi, Kenya<br />P.O. Box 52883 (00100)</span>
          </span>
        </div>
        <p className="font-inter text-[15px] leading-[1.75] text-[#2c4b46]"><a href="tel:+254787990220" className="text-[#2c4b46] underline underline-offset-[2px] hover:text-[#06332e]"><svg viewBox="0 0 24 24" className="mr-[6px] inline-block h-[13px] w-[13px] flex-shrink-0 align-[-2px] opacity-75" fill="none" stroke="currentColor" strokeWidth="1.7" strokeLinecap="round" strokeLinejoin="round"><path d="M6.6 10.8a15.5 15.5 0 0 0 6.6 6.6l2.2-2.2a1.5 1.5 0 0 1 1.5-.4c1.2.4 2.5.6 3.8.6a1.5 1.5 0 0 1 1.5 1.5V21a1.5 1.5 0 0 1-1.5 1.5C10.5 22.5 1.5 13.5 1.5 3.5A1.5 1.5 0 0 1 3 2h3.7a1.5 1.5 0 0 1 1.5 1.5c0 1.3.2 2.6.6 3.8a1.5 1.5 0 0 1-.4 1.5l-2.2 2Z" /></svg>+254 787 990 220</a><br /><a href="mailto:info@binaryone.co.ke" className="text-[#2c4b46] hover:text-[#06332e]">info@binaryone.co.ke</a></p>
        <button onClick={() => { setAdvisoryOpen(true); setAdvDone(false); setPartnersOpen(false); setError(""); setAdvConsent(false); setAdvErrEmail(false); setAdvErrPhone(false); setAdvErrConsent(false); }} className="inline-flex cursor-pointer items-center gap-[9px] self-start rounded-[9px] border border-[#00332f] bg-[#00332f] px-[13px] py-[9px] font-jet text-[10px] sm:text-[10.5px] font-bold tracking-[0.08em] sm:tracking-[0.1em] text-[#eafaf6] transition-[background,border-color] hover:border-[#0f766e] hover:bg-[#0a4a42] hover:text-white">
          ...
          <div className="rounded-[16px] border border-[#E5E7EB] bg-white p-[40px] shadow-[0_1px_2px_rgba(0,0,0,0.04),0_8px_24px_rgba(0,0,0,0.05)]">
            ...
            <form noValidate onSubmit={(e) => handleForm(e, submitAssessment)} className="flex flex-col gap-[16px]">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-[16px]">
                ...
                <input required name="phone" placeholder="Phone *" type="tel" inputMode="numeric" value={phone} onChange={(e) => { setPhone(e.target.value.replace(/[^0-9+ ]/g, "")); setErrPhone(false); }} className={FIELD} style={errPhone ? { borderColor: "#c1121f" } : undefined} />
        ...
        <div className="fixed inset-0 z-[70] flex items-center justify-center p-[24px] [background:rgba(3,20,18,0.66)] [backdrop-filter:blur(9px)]">
          ...
          <div data-adv-panel="1" className="relative flex max-h-[92vh] w-[760px] max-w-full flex-col gap-[13px] overflow-y-auto rounded-[20px] border border-[rgba(56,224,196,0.28)] bg-[#00332f] px-[28px] py-[22px] shadow-[0_40px_90px_rgba(0,0,0,0.55)]">
            ...
            {advDone ? (
              ...
              <form noValidate onSubmit={handleAdvisory} className="flex flex-col gap-[12px]">
                {HONEYPOT}
                ...
                <div className="grid grid-cols-2 lg:grid-cols-4 gap-[10px]">
                  ...
                  <label className="flex flex-col gap-[6px]"><span className={ADV_LABEL}>WORK EMAIL *</span><input required name="email" type="email" onChange={() => setAdvErrEmail(false)} placeholder="you@company.co.ke" className={ADV_FIELD} style={advErrEmail ? { borderColor: "#ff8a8a" } : undefined} /></label>
                  <label className="flex flex-col gap-[6px]"><span className={ADV_LABEL}>MOBILE NUMBER *</span><input required name="phone" type="tel" inputMode="numeric" onInput={(e) => { phoneNumeric(e); setAdvErrPhone(false); }} placeholder="+254 700 000 000" className={ADV_FIELD} style={advErrPhone ? { borderColor: "#ff8a8a" } : undefined} /></label>
                </div>

                <label className="flex flex-col gap-[6px]">
                  <span className={ADV_LABEL}>BRIEFING SCOPE</span>
                  <textarea name="agenda" rows={3} placeholder="What's your most pressing IT priority?" className={`${ADV_FIELD} resize-y leading-[1.55]`} />
                </label>

                <label className="flex cursor-pointer items-start gap-[9px] font-inter text-[12px] leading-[1.5]" style={{ color: advErrConsent ? "#ff8a8a" : "#8fb3ae" }}>
                  <input type="checkbox" checked={advConsent} onChange={(e) => { setAdvConsent(e.target.checked); setAdvErrConsent(false); }} className="mt-[2px] [accent-color:#38e0c4]" />
                  <span>I consent to Binary One Solutions processing this information under their <Link href={routes.dataProtection} className="text-[#9EFF5A] hover:text-[#c4ff9e]">Data Protection Policy</Link>.</span>
                </label>
                {advErrEmail && <p className="font-inter text-[12px] leading-[1.4] text-[#ff8a8a]">Enter a valid email address, including @.</p>}
                {advErrPhone && <p className="font-inter text-[12px] leading-[1.4] text-[#ff8a8a]">Enter a phone number we can reach you on.</p>}
                ...
                <p className="mt-[4px] font-inter text-[12px] leading-[1.5] text-[#8fb3ae]">Ninety minutes, no charge, no obligation.</p>
              </form>
            )}
          </div>
        </div>
